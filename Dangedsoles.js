...
var enemyHurtArray = [
  new Audio("Media/Audio/Ow1.mp3"),
  new Audio("Media/Audio/Ow2.mp3"),
  new Audio("Media/Audio/Ow3.mp3"),
  new Audio("Media/Audio/Ow4.mp3"),
];
// enemy list
var EnemyList = {
  1: {
...
var WeaponList = {
  ...
  2: {
    ...
    image_default: "Media/images/SharpStick.png",
    image_hitting: "Media/images/SharpStick.gif",
    damage: 2,
  },
  3: {
    name: "Banished Saber",
...
function summon_enemy(name) {
  ...
  document
    ...
    .insertAdjacentHTML(
      "beforebegin",
      `<progress id="enemy-health" value="${EnemyList[enemy_integer].health}" max="${EnemyList[enemy_integer].health}"></progress><p id="health-label"><span id="current-health">${current_enemy_health}</span>/${EnemyList[enemy_integer].health}</p> <p id="enemy-name">${EnemyList[enemy_integer].name}</p><div id="warning"><p id="warning-message"></p></div>`
    );
}
...
function damageEnemy() {
  ...
  document.getElementById("click_count").innerHTML =
    numberWithCommas(click_count);
  document.getElementById("weapon-image").src =
    WeaponList[weapon_integer].image_hitting;
  setTimeout(function () {
    document.getElementById("weapon-image").src =
      WeaponList[weapon_integer].image_default;
  }, 400);

  document.getElementById("enemy-image").src =
    EnemyList[enemy_integer].image_hurt;
  setTimeout(function () {
    document.getElementById("enemy-image").src =
      EnemyList[enemy_integer].image_dancing;
  }, 200);

  current_enemy_health -=
    WeaponList[weapon_integer].damage * weapon_damage_modifier;
  ...
  if (document.getElementById("enemy-health").value <= 0) {
    ...
    document.getElementById("soles_count").innerHTML =
      numberWithCommas(soles_count);
    enemy_integer++;
...
function open_shop() {
  ...
  document.getElementById("shop-container-background").style.visibility =
    "visible";
}
...
function purchaseItem(item) {
  switch (item) {
    case "sharp-stick":
      if (soles_count >= 50 && sharpstick == false) {
        weapon_integer = 2;
        document.getElementById("weapon-image").src =
          WeaponList[weapon_integer].image_default;
        soles_count -= 50;
      ...
      break;
    case "sharpness":
      if (click_count >= 300 && sharpness == false) {
